...
import { fetchPDFs, reprocessPDF, deletePDF } from "@/services/pdfService";
    ...
    <Modal open={open && !!pdf} onClose={onClose} panelClassName="sm:max-w-sm">
      <div className="p-6">
        ...
        <p className="mt-2 text-sm text-ink-600">
          ...
          এবং এর সাথে সম্পর্কিত ভোটার রেকর্ডগুলো সার্চ থেকে মুছে ফেলা হবে। ফাইলটি
          এডমিনের কাছে সংরক্ষিত থাকবে।
        </p>
      </div>
...
export default function PDFListPage() {
  ...
  const { me, status: permStatus } = useCurrentUser();
  const canView =
    permStatus !== "ok" ? true : hasPermission(me, "canViewPDF");

  // Every row in this list belongs to the logged-in user, so they can always
  // manage (reprocess / delete) their own files.
  ...
  if (!canView) {
    return (
      <PermissionDenied
        title="পিডিএফ তালিকা"
        description="পিডিএফ তালিকা দেখার অনুমতি নেই। কমপক্ষে 'পিডিএফ তালিকা দেখা' পারমিশন প্রয়োজন।"
      />
    );
  }
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <motion.div variants={fadeUp} initial="hidden" animate="visible">
        <h1 className="text-xl font-bold text-ink-900 sm:text-2xl">পিডিএফ তালিকা</h1>
        <p className="mt-1 text-xs text-ink-600 sm:text-sm">
          আপলোড করা PDF ফাইল ও তাদের প্রক্রিয়াকরণ অবস্থা
        </p>
      </motion.div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2 py-16 text-ink-600">
              ...
              <Link href="/dashboard/upload" className="text-sm font-medium text-brand-700 hover:underline">
                  ...
                  <motion.div key={pdf.id} variants={fadeUp} className="p-3 sm:p-4">
                    <div className="overflow-hidden rounded-xl border border-ink-200 border-l-4 border-l-brand-400 bg-white p-4">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-2 border-t border-ink-100 pt-3">
                        <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${STATUS_BADGE_CLASS[pdf.processingStatus] || ""}`}>
                      ...
                      <motion.tr key={pdf.id} variants={fadeUp} className="row-hover transition-colors">
                        <td className="min-w-0 px-3 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="truncate font-medium text-ink-900">{pdf.fileName}</span>
                          </div>
                        </td>
                        <td className="min-w-0 px-3 py-3 text-xs text-ink-600">{pdf.uploaderName}</td>
                        <td className="px-3 py-3 text-xs text-ink-600">{fileSizeLabel(pdf.fileSize)}</td>
                        <td className="px-3 py-3 text-xs text-ink-600">
                          {pdf.totalRecords ?? 0}
                        </td>
                        <td className="px-3 py-3">
                          <div className="flex flex-col items-start gap-1">
                            <span className={`inline-block whitespace-normal rounded-full px-2 py-0.5 text-[10px] font-semibold ${STATUS_BADGE_CLASS[pdf.processingStatus] || ""}`}>
                              {STATUS_LABELS[pdf.processingStatus] || pdf.processingStatus}
                            </span>
                            {pdf.deletedAt && (
